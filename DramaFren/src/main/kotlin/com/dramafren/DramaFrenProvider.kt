...
class DramaFrenProvider : MainAPI() {
    ...
    private fun cfHeaders(url: String): Map<String, String> {
        ...
        DramaFrenStore.getCfCookieForUrl(url)?.let { ck ->
            base["Cookie"] = ck
        }
        return base
    }
    ...
    private suspend fun fetchCards(url: String): List<MovieSearchResponse> {
        ...
        for (a in doc.select("a[href*=\"/drama/\"]")) {
            ...
            out += newMovieSearchResponse(title, abs, TvType.AsianDrama) {
                this.posterUrl = poster
            }
        }
        // Fallback: if selector missed due to RSC streaming, try regex on html
        if (out.isEmpty()) {
    ...
    data class EpisodeJson(val episode: Int? = null, val episodeId: String? = null, val title: String? = null)
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // episodes: a.episode-link href="/watch/...?ep=N"
        ...
        // fallback: if no links found, try to find JSON in page with ep count
        val episodes = if (eps.isNotEmpty()) eps else {
            // try to extract from HTML: "EP 60" etc.
    ...
    override suspend fun loadLinks(
        ...
        // Try to find JSON with video sources in page
        // Look for m3u8/mp4 links, subtitles, audio
        ...
        // Also try to find JSON like "url":"https://...","quality":"1080p"
        ...
        // subtitles: "subtitle","label":"English","url":"https://..."
        ...
        // Fallback: try to find video src in <video> or <source>
        val doc = Jsoup.parse(html, DramaFrenStore.apiBase())
        ...
        // If still nothing, try to fetch via API endpoint that watch page's JS would call
        // Heuristic: look for "/api/" in html
