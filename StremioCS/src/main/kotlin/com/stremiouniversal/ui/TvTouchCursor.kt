...
class TvTouchCursor(
    ...
    private val cursorSize = (28 * density).toInt()
    private val stepSlow = (10 * density).toInt()
    private val stepFast = (28 * density).toInt()
    ...
    private val cursorView: View = View(context).apply {
        background = GradientDrawable().apply {
            ...
            setStroke((2 * density).toInt(), Color.parseColor("#22C55E"))
        }
    ...
    fun show() {
        isActive = true
        if (x == 0f && y == 0f) {
            x = if (overlay.width > 0) overlay.width / 2f else 300f
            y = if (overlay.height > 0) overlay.height / 2f else 300f
        }
