 ...
 *   Row 1: "Domain" label + current domain text
 *   Row 2: single EditText to edit/replace it
 *   Row 3: [Save]  [Solve Cloudflare] + cookie status line
 *
 * Flow: current -> edit -> Save persists via Store and reloads home.
 */
object VdramaSettingsDialog {
    ...
    fun show(context: Context, currentBaseUrl: String) {
        ...
        val status = TextView(context).apply {
            val hasCf = VdramaStore.getCfCookieForUrl(currentBaseUrl) != null
        ...
        val cfButton = Button(context).apply {
            ...
            setOnClickListener {
                val target = normalizeBaseUrl(input.text?.toString()) ?: currentBaseUrl
    ...
    // ---- shared helpers (copy-identical in all 4 providers) ----
    ...
    private fun TextView.updateCf(url: String) {
        val has = VdramaStore.getCfCookieForUrl(url) != null
